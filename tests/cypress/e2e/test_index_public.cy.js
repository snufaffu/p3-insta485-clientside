...
describe("Index page when awdeorio is logged in with mock database", () => {
  ...
  it("Updates human-readable timestamps every minute without page refresh", () => {
    // Freeze time
    const now = new Date("2022-09-18T09:18:36Z");
    cy.clock(now.getTime(), ["setInterval", "setTimeout", "Date"]);

    // Visit the main page.  Run a callback before the page's JavaScript
    // executes to wrap setInterval() with a "spy" that records calls to the
    // real setInterval().
    cy.visit("/", {
      onBeforeLoad(win) {
        cy.spy(win, "setInterval").as("intervalSpy");
      },
    });

    // Wait for REST API request to complete.  This is for the list of posts.
    cy.wait("@getPosts");

    // Verify that a post exists
    cy.get("[data-testid='post-time-ago']");

    // Verify initial timestamp for the new post
    cy.get("[data-testid='post-time-ago']")
      .first()
      .invoke("text")
      .should((text) => {
        // Should be very recent, e.g., "a few seconds ago"
        expect(text).to.match(/seconds ago|a few seconds ago/);
      });

    // Verify that the student code called setInterval()
    cy.get("@intervalSpy").should("have.been.called");

    // Advance time.  This should trigger the student's callback that was set
    // by the student call to setInterval().
    cy.tick(60000);

    // Verify that the first post's human-readable timestamp has updated
    cy.get("[data-testid='post-time-ago']")
      .first()
      .invoke("text")
      .should((text) => {
        expect(text).to.match(/a minute ago|1 minute ago/);
      });
  });
});
